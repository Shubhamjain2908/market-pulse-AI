...
| **Screen** | `src/analysers/stock-screener.ts` | Loads `config/screens.json`. DSL screens via `engine.ts`; **`quality_garp`** and **`catalyst_entry`** use dedicated dispatchers (`getQualityGarpFundamentals` + **13 gates** including OPM stability, pledge shadow, and QDS; `runCatalystScreener`). Checks `regime_strategy_gate`. Writes passing symbols to `screens`. Gates 3/7/8/9 are **regime-aware** via `resolveGarpThresholds(regime)` in `quality-garp.ts`. |
| **AI Thesis** | `src/agents/thesis-generator.ts` | Candidate pool: today's `screens` hits ∪ watchlist, minus holdings and OPEN `paper_trades`. Ranks by enriched signals + screen/alert matches; sends technicals, fundamentals, exact-symbol news, screens, and regime context to LLM. Untagged news and FII/DII are excluded from stock context and provenance; market-wide flows remain in Market Mood. Parallel via `p-limit` (`THESIS_CONCURRENCY`, default 3). |
| **Concall fetch** | `src/ingestors/bse/concall-fetcher.ts` | After pledge, before enrich: fail-open pipeline stage `concall-fetch`. For holdings ∪ open paper trades ∪ watchlist, spawns Python subprocess `fetch-bse-concall.py` per symbol that uses `BseIndiaApi` to resolve scrip code and download the latest "Earnings Call Transcript" PDF from BSE. Extracts text via `unpdf`, inserts into `concall_transcripts`. Skips PDFs with <2000 chars (likely image-only). Gated by `CONCALL_ANALYSIS_ENABLED='1'` (code default `'0'`; production may override). Never throws — returns result counters. |
...
**Phase gate:** GTT execution is **gated observe mode** until all four gates in [`docs/gtt-activation-criteria.md`](../../docs/gtt-activation-criteria.md) pass sequentially (≥30 post-fix closed trades with `source_date >= 2026-05-14`, positive post-cost expectancy and profit factor >1.2 per signal type, clean qualitative audit, live `regime_daily = BULL_TRENDING` at activation). Pre-fix dedup baseline expectancy remains negative — not comparable to backtest.
...
Status: **v3 shipped (2026-07-06)** — `pnpm fundamentals:refresh` orchestrates Python annual backfill (241 symbols), screener ingest, then Yahoo snapshot (yahoo wins on `(symbol, as_of)` conflict). **`quality_garp`** has **13 gates** in code order: ETF exclusion; fundamentals with PE/PB; PE/PB ceilings; 3yr ROE; ROCE; D/E; PEG; RSI; SMA50; promoter selling; **promoter pledge** (gate 11, shadow by default); **OPM stability** (gate 12, fail-open <4Q); and **Quality Decay Score** (gate 13, hard block ≤3, soft warn =4, fail-open <5Q, bypassed in CRISIS). Gates 3/7/8/9 use regime-aware PE/PEG/RSI/SMA50 thresholds. Funnel counters: `qds`/`qds_warning`/`qds_skipped`/`pledge_shadow`/`opm_skipped`.
...
| **AI_PICK earnings blackout (fail-closed)** | Block AI_PICK admission when `mom_earnings_blackout=1` (active blackout) **or** when blackout signal is missing/stale (unknown — fail-closed). Freshness uses T-2 session boundary (same as rank/false-flag). `catalyst_entry` remains exempt. Signal loaded in `evaluateAiPickEligibility` **before** Path A/B/C gates so event-risk blocks all admission paths. | `src/briefing/ai-pick-gate.ts` |
