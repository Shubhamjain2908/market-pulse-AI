...
Status: **v2 shipped (2026-06-06)** — `pnpm fundamentals:refresh` orchestrates Python annual backfill (241 symbols), screener ingest, then Yahoo snapshot (yahoo wins on `(symbol, as_of)` conflict). **`quality_garp`** gates (11 total): 3yr ROE≥18%, ROCE≥20%, D/E<0.5, PEG<1.2, PE/PB ceilings, RSI+SMA50 dip, promoter selling block, plus **OPM stability** (gate 11) — trailing 4Q `opm_pct` std-dev ≤ 5% via `getTrailingOpmStdDev`; fail-open when <4 quarters available (`opm_skipped` funnel counter tracks fail-open hits).
