...
| **Screen** | `src/analysers/stock-screener.ts` | Loads `config/screens.json`. DSL screens via `engine.ts`; **`quality_garp`** and **`catalyst_entry`** use dedicated dispatchers (`getQualityGarpFundamentals` + 11 v2 gates including OPM stability; `runCatalystScreener`). Checks `regime_strategy_gate`. Writes passing symbols to `screens`. |
...
Status: **v2 shipped (2026-06-06)** — `pnpm fundamentals:refresh` orchestrates Python annual backfill (241 symbols), screener ingest, then Yahoo snapshot (yahoo wins on `(symbol, as_of)` conflict). **`quality_garp`** gates (12 total): 3yr ROE≥18%, ROCE≥20%, D/E<0.5, PEG<1.2, PE/PB ceilings, RSI+SMA50 dip, promoter selling block, **promoter pledge ≤15%** (gate 12 — shadow by default via `QUALITY_GARP_PLEDGE_GATE=0`; `pledge_shadow` funnel counter), plus **OPM stability** (gate 11) — trailing 4Q `opm_pct` std-dev ≤ 5% via `getTrailingOpmStdDev`; fail-open when <4 quarters available (`opm_skipped` funnel counter tracks fail-open hits).

**Promoter pledge ingest:** Daily fail-open `pledge` stage after `corporate-actions` fetches NSE `/api/corporate-pledgedata?index=equities`, resolves `comName` → symbol via `symbols.name`, persists `promoter_pledge`. Portfolio guardrails flag QoQ pledge rise and high pledge (>15%) advisory-only.
...
**Deferred:** `operating_margin_pct` annual column on `fundamentals`; Dec-FY `as_of` edge cases.
