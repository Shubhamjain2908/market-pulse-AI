...
**Pipeline audit (`pipeline_runs`, migration `0022`):** Each stage in `daily-workflow.ts` appends `started` / `success` / `failed` / `skipped` via `recordPipelineStage` in `src/db/pipeline-queries.ts`. Fail-open stages (portfolio sync, corporate actions, Yahoo snapshot, **momentum-rank**, ext-signal, iNAV, Sunday earnings) log `failed` but do not abort; fatal stages rethrow. Stage names match the table (`ingest`, `enrich`, `regime`, `screen`, `evaluate`, `briefing`, etc.). `hasFailedRequiredStage(run_date)` / `listFailedRequiredStages` use the **latest row per stage** (`ROW_NUMBER` over `id DESC`) for **`enrich`**, **`regime`**, **`screen`** — a successful retry on the same `run_date` clears degraded mode. When any required stage's latest status is `failed`, the briefing composer renders a **degraded** brief (banner + mood/regime if regime's latest status succeeded; omits screens, thesis, portfolio, momentum).
