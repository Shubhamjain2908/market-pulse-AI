...
Status: **v2 shipped (2026-06-06)** — `pnpm fundamentals:refresh` orchestrates Python annual backfill (241 symbols), screener ingest, then Yahoo snapshot (yahoo wins on `(symbol, as_of)` conflict). **`quality_garp`** gates (13 total): 3yr ROE≥18%, ROCE≥20%, D/E<0.5, PEG<1.2, PE/PB ceilings, RSI+SMA50 dip, promoter selling block, **promoter pledge ≤15%** (gate 12 — shadow by default via `QUALITY_GARP_PLEDGE_GATE=0`; `pledge_shadow` funnel counter), plus **OPM stability** (gate 11) — trailing 4Q `opm_pct` std-dev ≤ 5% via `getTrailingOpmStdDev`; fail-open when <4 quarters available (`opm_skipped` funnel counter tracks fail-open hits), and **Quality Decay Score (QDS)** (gate 13) — 6-signal trajectory score (0–6) from `quarterly_fundamentals`; hard block at ≤ 3, soft warn at =4, fail-open when <5 quarters, bypassed in CRISIS (`qds`/`qds_warning`/`qds_skipped` funnel counters).
