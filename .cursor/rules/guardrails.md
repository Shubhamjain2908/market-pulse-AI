...
| **Quality-GARP hard null block** | `quality_garp` must hard-block symbols when any required gate input is missing (`pe`, `pb`, `prev_roe`, `latest_rev_growth`, `sma_50`, `close`); only `promoter_holding_change_qoq` is fail-open on NULL | `src/analysers/stock-screener.ts` |
