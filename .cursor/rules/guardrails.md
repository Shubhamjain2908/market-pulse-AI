...
| **Option A backtest regime gate** | Walk-forward runner requires ≥80% of benchmark trading days in `[from, to]` to have a `regime_daily` row; otherwise abort with coverage stats and point to `scripts/backfill-regime.mts`. Momentum factors use `adj_close`-aligned series (same as live enrich). Debug label skew with `scripts/audit-regime-history.mts`. | `src/backtest/runner.ts`, `src/enrichers/momentum-signals.ts` |